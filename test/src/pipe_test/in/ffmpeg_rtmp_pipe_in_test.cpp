...
void test_read(){
    ...
    context.iterate_frame_test_cases(test_name, "rgb_palette0/1280x720", [&](ITER_ACTION_ARGS){
        ...
        uint8_t *data1 = new uint8_t[frame_size];
    ...
    ffmpeg_rtmp_pipe_in *pipe_in = new ffmpeg_rtmp_pipe_in(config);
    ffmpeg_rtmp_pipe_out *pipe_out = new ffmpeg_rtmp_pipe_out(config);
    bool success = true;
    int fail_index = -1;
    thread out([&](){
    ...
    for(int i = 0; i < frame_count && success; i++){
        auto frame = frames[i];
        uint8_t *frame_received = pipe_in->read(frame_size);
