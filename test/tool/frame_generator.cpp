...
void write_expected(frame_meta config, string expected_name){
    path context_path = path("..") / path("..") / path("test") / path("context");
    path expected_path = context_path / path("expected") / path(expected_name);
    ...
    string str = j.dump();
    fstream blocks_file(expected_path, ios_base::out);
